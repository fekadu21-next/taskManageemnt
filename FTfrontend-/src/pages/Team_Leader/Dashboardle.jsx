...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const channel = echo.private(`tasks.${user.id}`);

    // ✅ Listen for the Laravel event name (must match backend)
    channel.listen("TaskAssigned", (data) => {
      console.log("🔔 New task received:", data);

      // Increment new task badge
      setNewTasksCount((prev) => (prev ? prev + 1 : 1));

      // Optional: sound or alert
  ...
  useEffect(() => {
    const saved = localStorage.getItem("newTasksCount");
    if (saved) setNewTasksCount(Number(saved));
  }, []);

  useEffect(() => {
    if (newTasksCount != null) {
      localStorage.setItem("newTasksCount", newTasksCount);
    }
  }, [newTasksCount]);
