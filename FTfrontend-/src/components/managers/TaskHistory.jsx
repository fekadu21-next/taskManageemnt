...
export default function TaskHistory() {
  ...
  useEffect(() => {
    const fetchHistories = async () => {
      ...
      try {
        ...
        let url = `${API_URL}/api/task-history`;
        let params = [];
        if (filter === "my") {
          params.push("my=true");
        } else if (filter === "team" && selectedTeam) {
          params.push(`team_id=${selectedTeam}`);
        } else if (filter === "all") {
  ...
  // Client-side filter for search and date
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto bg-gray-100 rounded-lg">
      {/* Header */}
      ...
      {/* Filters */}
      <div className="flex flex-col md:flex-row gap-1 items-center justify-between -ml-6">
      ...
      {/* Team filter requires selection */}
      {filter === "team" && !selectedTeam && (
        <p className="text-gray-500 text-center py-6">
          Please select a team to view its task history.
        </p>
      )}
      {/* Content */}
