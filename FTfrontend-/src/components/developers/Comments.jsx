...
export default function Comments({ recentCount = 3 }) {
  ...
  useEffect(() => {
    ...
    const fetchTasks = async () => {
      try {
        const res = await fetch(`${API_URL}/Devtasks`, {
          headers: authHeaders,
        });
        const data = await res.json();
      ...
      <div className="flex items-center justify-between mb-6 ">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center cursor-pointer gap-2 bg-slate-400 text-white px-4 py-2 rounded-xl hover:bg-slate-500"
          >
            <FiPlus /> Add
          </button>
        </div>
