...
export default function Comments({ recentCount = 3 }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUsers(safeData);
        // ✅ Only set default recipient to a valid user (not self)
        const validRecipients = safeData.filter((u) => u.id !== user?.id);
        if (validRecipients.length && !formRecipientId) {
          setFormRecipientId(validRecipients[0].id);
        }
      } catch (err) {
    ...
    const fetchComments = async () => {
      try {
        const res = await fetch(`${API_URL}/api/comments/all`, {
          headers: authHeaders,
        });
        const data = await res.json();
        const safeData = Array.isArray(data) ? data : data.data || [];
  ...
  const handleCreateComment = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/tasks/${formTaskId}/comments`, {
        method: "POST",
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      ...
      // Optional: auto-clear success message after few seconds
      setTimeout(() => setSuccessMessage(""), 3000);
    } catch (err) {
      console.error(err);
    }
  ...
  const downloadFile = async (file) => {
    try {
      const res = await fetch(
        `${API_URL}/api/attachments/${file.id}/download`,
        {
          headers: authHeaders,
        }
      );
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-white border hover:bg-gray-50 cursor-pointer"
          >
            {layout === "grid" ? <FiList /> : <FiGrid />}
          </button>
          <button
            onClick={() => setOpenModal(true)}
            className="flex items-center gap-2 bg-slate-400 text-white cursor-pointer px-4 py-2 rounded-xl hover:bg-slate-500"
          >
            <FiPlus /> Add
          </button>
        </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-xl font-semibold flex items-center cursor-pointer gap-2">
              ...
              <div className="flex items-center justify-between">
                <label className="cursor-pointer flex items-center gap-2 text-gray-600 hover:text-indigo-800">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="bg-slate-400 text-white cursor-pointer px-4 py-2 rounded-xl hover:bg-slate-500 flex items-center gap-2"
                  >
                    <FiSend /> Sent Comment
                  </button>
                </div>
